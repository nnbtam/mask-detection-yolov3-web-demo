...
APP_ROOT = os.path.dirname(os.path.abspath(__file__)) + "/yolo-coco/"

# load the COCO class YOLO model was trained on
labelsPath = os.path.join(APP_ROOT, "coco.names")

# derive the paths to the YOLO weights and model configuration
weightsPath = os.path.join(APP_ROOT, "yolov3.weights")
	...
	if len(idxs) > 0:
		for i in idxs.flatten():
			...
			if (classes[classIDs[i]] == "face"):
				cv2.putText(image, "WEAR THE MASK", (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
				1.0, (0,0,255), 3)
